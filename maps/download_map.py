...
def parse_arguments():
    ...
    parser.add_argument("--radius", type=validate_radius, default=15,
                       help="Search radius in kilometers (default: 15)")
...
def build_map(lat, lng, zoom, style, sanitized_title, osm_geojson, trailheads_geojson):
    ...
    if osm_geojson and osm_geojson.get("type") == "FeatureCollection" and osm_geojson.get("features"):
        folium.GeoJson(
            ...
            tooltip=folium.features.GeoJsonTooltip(fields=["highway"], aliases=["Type"])
        ).add_to(hiking_fg)
        ...
        for f in osm_geojson["features"]:
            ...
            if props:
                ...
                highway_type = props.get('highway', 'trail')
                
                # Build info content dynamically based on available properties
                info_parts = [f"<b>{name}</b>"]
                info_parts.append(f"<b>Type:</b> {highway_type}")
                
                if props.get('sac_scale'):
                    info_parts.append(f"<b>SAC:</b> {props['sac_scale']}")
                if props.get('surface'):
                    info_parts.append(f"<b>Surface:</b> {props['surface']}")
                if props.get('network'):
                    info_parts.append(f"<b>Network:</b> {props['network']}")
                if props.get('leisure'):
                    info_parts.append(f"<b>Leisure:</b> {props['leisure']}")
                if props.get('natural'):
                    info_parts.append(f"<b>Natural:</b> {props['natural']}")
                
                html_content = "<br>".join(info_parts)
                folium.Popup(html_content, max_width=320).add_to(hiking_fg)
    else:
        print("No OSM trail data found in the specified area.")
...
def fetch_osm_data(lat, lng, radius_km):
    ...
    (
      ...
      way["highway"="cycleway"]["foot"="yes"]({south},{west},{north},{east});
      ...
      way["natural"="trail"]({south},{west},{north},{east});
    );
    out geom;
    """
    ...
    try:
        ...
        if data.get("elements"):
            print(f"Found {len(data['elements'])} OSM trail elements")
            ...
            print(f"Converted to {len(geojson_data['features'])} trail features")
            return geojson_data
        else:
            print("No trail elements found in OSM response")
